...
def penalty_pso_function(parameter_values, bounds, n_particles, m_iterations, inertia, cognitive, social):
    ...
    print("PSO Penalty Algorithm Started")
    ...
    for iter in range(max_iterations):
        ...
        for i in range(num_particles):
            ...
            fitness = objective_function1(temp)

            # Update personal best
            temp = unflatten(pbest[i], parameter_values)
            if fitness > objective_function1(temp):
                pbest[i] = current_position[:]
...
def objective_function1(para):
   ...
   condition_14 = P_21 <= P_dash_21_max
   condition_15 = P_dash_12 <= P_dash_12_max

   if not condition_1:
       penalty = penalty + rate_D11 - R_11_th
   ...
   ## Recalculate rates based on these values.
   return rate_D11 + rate_D11 + rate_D21 +  rate_D22  -0.1*penalty

a = penalty_pso_function(par, bounds, n_particles, m_iterations, inertia, cognitive, social)

plt.plot(np.arange(len(a)), a, label='PSO with Penalty', color='green', linestyle='--')
